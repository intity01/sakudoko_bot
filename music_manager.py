...
from player import YTDLSource, YTDL_INSTANCE as invidious
...
class MusicManager:
        ...
        if not self.queue:
            if self.auto_play:
                ...
                try:
                    ...
                    info = await invidious.extract_info(keyword, download=False)
                    urls = []
    ...
    async def handle_music_request(self, message: discord.Message, query: str):
        ...
        try:
            # 1. Extract info (async operation with Invidious)
